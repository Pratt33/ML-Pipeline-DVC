...
def load_parameters(path="params.yaml"):
    try:
        ...
        print(f"Error: Parameter file {path} not found")
        sys.exit(1)
...
def use_parameters():
    try:
        ...
        print(f"Error: Missing parameter {e} in params.yaml")
        sys.exit(1)
...
def filter_data(df: pd.DataFrame, min_user_interactions: int, min_product_interactions: int) -> pd.DataFrame:
    try:
        ...
        print(f"Error: Missing required column {e} for filtering")
        sys.exit(1)
...
def extract_top_summaries(df):
    try:
        ...
        print(f"Error extracting summaries: {e}")
        sys.exit(1)
...
def save_model(model, user_product_matrix, top_summaries, output_model):
    try:
        ...
        print(f"Error saving model to {output_model}: {e}")
        sys.exit(1)
...
def main():
    try:
        ...
        print(f"Model training completed. Model saved to: {output_model}")
